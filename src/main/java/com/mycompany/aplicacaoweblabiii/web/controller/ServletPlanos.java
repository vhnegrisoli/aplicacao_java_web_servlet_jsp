 ...
 * @author vhnegrisoli
 */
public class ServletPlanos extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet ServletPlanos</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet ServletPlanos at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (!id.trim().equals("null")) {
            Plano plano = new Plano(Integer.parseInt(id));
            plano.setIdPlano(Integer.parseInt(id));
            plano.setDescricao(nome);
            plano.setPreco(preco);
            try {
                pjc.edit(plano);
                response.sendRedirect("PlanosUD.jsp");
            } catch (Exception ex) {
