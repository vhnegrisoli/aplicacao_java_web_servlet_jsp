 ...
 */
public class ServletPagamentoDelete extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet ServletPagamentoDelete</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet ServletPagamentoDelete at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        System.out.println("PK encontrada: " + id.trim());
                
        try {
            EntityManagerFactory emf = Persistence.createEntityManagerFactory("com.mycompany_AplicacaoWebLabIII_war_1.0-SNAPSHOTPU");
            FormaPgtJpaController fjc = new FormaPgtJpaController(emf);
            fjc.destroy(Integer.parseInt(id.trim()));
            erro = false;
        } catch (Exception e) {
            e.printStackTrace();
            erro = true;
